...
class TodayScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final missions = character.dailyQuests
        .where((q) => q.scheduledDay == today)
        .toList();
              ...
              Row(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(
                      focusIcon(quest.template.focus),
                      color: color,
                      size: 20,
                    ),
                  ),
                  ...
                  IconButton(
                    ...
                    onPressed: () => _accept(context, quest, locale),
                  ),
                ],
              ),
              if (quest.generatedFor(locale) && quest.instruction != null)
                Padding(
                  padding: const EdgeInsets.only(left: 47, top: 8, right: 8),
                  child: Text(
                    quest.instruction!,
                    maxLines: 2,
      ...
      child: ListTile(
        ...
        leading: IconButton(
          ...
          onPressed: quest.isCompleted
              ? null
              : () => _showActive(context, quest),
          icon: Icon(
            quest.isCompleted
                ? PhosphorIcons.checkCircleFill
                : PhosphorIcons.circle,
            color: quest.isCompleted ? colors.primary : colors.onSurfaceVariant,
            size: 25,
          ),
        ),
        title: Text(
          ...
          children: [
            ...
            Text(
              quest.generatedFor(locale)
                  ? quest.reason!
                  : (quest.recovery ? s.lqRecoveryReason : s.lqDefaultReason),
              style: theme.textTheme.bodySmall,
            ),
