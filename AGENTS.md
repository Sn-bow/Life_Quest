...
> **현재 작업 (2026-09-20):** `codex/rebirth-2026-09`에서 Life Quest 2.0 재구축 중.
...
> 사용자 최신 지시: 이미지 아트는 image_gen 또는 라이선스가 확인된 무료 이미지/템플릿을 사용. SVG/HTML/Canvas로 직접 그린 아트 금지. 이번 회차는 최대 30%만 사용하고 잔여 70%를 남긴다(2026-09-20 사용자 재확인).
> 최신 기획 확정: Life Quest: 경계의 서가. 도시·수련·탐사의 세 무료 단편과 기록자 상태창. `docs/rebirth/CONCEPT_AND_REVENUE.md`가 콘셉트·수익 기준이다. 별도 완결 세계관 팩6,900원은 검증 후보이며 「조수 우체국」12장면×4언어·두 결말·테마·표식·두 장면 무료 체험은 구현했다. 독자 검수/실제 Play 상품 등록·판매는 아직 없다.
> 최신 main 검토 빌드2.0.0+5(소스489ccff): 삭제 복구·조수 팩·cloud snapshot 저장 수정 포함. Flutter전체337+추가캠페인6, 별도결제카탈로그1, 서버27검사 통과. 실제 기기/거래/Play 배포와 구분한다.
