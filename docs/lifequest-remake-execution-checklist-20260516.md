...
- [ ] 칭호가 추천 행동/이벤트 조건으로 작동하는 1차 연결 구현
...
1. [ ] title metadata 확장 가능성 조사
2. [ ] 별도 `title_unlock_rules.dart`로 시작할지 결정
3. [ ] 다음 칭호 진행도 계산
4. [ ] 오늘의 모험 블록에 다음 칭호 표시
5. [ ] 던전 이벤트 조건 선택지 1개 파일럿
6. [ ] 칭호 효과 설명 UI 추가
...
- [ ] 사용자가 칭호를 얻어야 하는 이유가 생김
- [ ] 칭호가 최소 1개 이상 실제 이벤트 선택지에 영향을 줌
