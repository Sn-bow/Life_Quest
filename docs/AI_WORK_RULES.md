...
이 Codex 대화 환경에서 새 게임 이미지를 생성할 때는 내장 이미지 생성 기능을 우선 사용한다.

- `OPENAI_API_KEY`를 사용자에게 요구하지 않는다.
- 로컬 imagegen CLI 또는 외부 API 키 기반 생성은 사용자가 명시적으로 요청하거나, 내장 이미지 생성 기능으로 처리할 수 없는 경우에만 검토한다.
- 생성 이미지는 기본 저장 위치에 남겨두고, 프로젝트에 사용할 때는 필요한 경로로 복사한다.
- 카드, 몬스터, 플레이어, 전투 이펙트 등 최종 게임 비주얼은 단순 도형 placeholder 대신 내장 이미지 생성 결과 또는 기존 PNG 에셋을 사용한다.
- 이미지 안에는 카드명, 비용, 설명, 수치, 다국어 텍스트를 넣지 않는다. 텍스트는 Flutter UI가 렌더링한다.
...
- 던전/전투 화면에서 별도의 플레이어 캐릭터, 아바타, 전신 스프라이트, 프로필 스프라이트를 다시 만들지 않는다.
- Life Quest의 전투 주체는 현실 퀘스트를 완료해 성장한 사용자 본인이다.
- `CharacterState`/`Character`는 현재 XP, 골드, 덱, 장비 보정 등 계정 진행 데이터 저장소로만 취급한다.
- UI 문구는 캐릭터명이 아니라 `나의 진행`, `퀘스트 성장`, `던전 보정`처럼 사용자 중심으로 표현한다.
