...
- Supplying `ADMOB_ANDROID_APP_ID` changes the manifest merge rule so the Mobile Ads SDK can declare the advertising ID permissions required by that build.
...
| Advertising ID / ad interactions | No in default build | N/A | Runtime startup is disabled and the final merged manifest excludes advertising-related SDK permissions; re-review if monetization is enabled |
