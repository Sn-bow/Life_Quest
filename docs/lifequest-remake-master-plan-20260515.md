...
## 2026-05-17 추가 피드백 반영

추가 Threads 피드백을 반영해 리메이크의 중심 문장을 수정한다.

```text
Life Quest는 던전을 플레이하기 위한 퀘스트 앱이 아니다.
현실 행동과 상태 체크를 RPG식 성장, 보상, 칭호, 추천 행동으로 번역하는 앱이다.
던전은 그 성장을 체감하는 선택형 보조 루프다.
```

따라서 전체 리메이크는 "던전 완성도"보다 "현실 행동이 앱 안에서 어떤 성장과 다음 행동으로 바뀌는지"를 먼저 완성한다. AR, 모션 인식, Health Connect, 실시간 PVP는 매력적인 후보지만 1차 리메이크 범위가 아니다. 수동 기록 기반 루프가 명확해지고 나서 확장한다.

계획 우선순위도 아래처럼 조정한다.

- P0: 오늘의 모험을 현실 행동 상태판으로 확장
- P0: 현실 행동 카테고리 -> 성장 -> 추천 행동 -> 선택형 던전 보정 연결
- P0: 던전 시작 시 오늘 보정 스냅샷 저장 및 일부 전투 수치 적용
- P1: 칭호/이벤트/현실 보상 연결
- P1: 주간 기록 기반 경쟁/챌린지 설계
- P2: AR, Health Connect, 모션 인식, 실시간 경쟁
