...
이 단계 이후 유료 본편/체험/판매 동선과 legacy cloud 삭제 복구를 구현했다(아래 및 CONTINUE 참조). 남은 품질 범위는 실물 Android 입력·TalkBack·모델 성능,5구역 게임 밸런스, 실제 독자/원어민 검수, 실제 결제/서버 검증이다. 스크린샷이나300개 테스트만으로 ‘완벽’ 또는 출시 승인이라고 주장하지 않는다.

## 첫 팩 체험 흐름 추가 검토

1. 서가: 세 무료 단편 아래 별도 「완결 이야기 팩」 영역. 버튼을 ‘처음 두 장면 무료 체험’으로 명확히 했다.
2. 읽기: 실제 CUA에서 첫 선택(open) → 두 번째 장면의 자전거 지도 반응 → 두 번째 선택(keep) 확인. 체험 중 XP는 추가되지 않는다.
3. 체험 종료: 다음 유료 장면으로 넘어가지 않고 팩 상세로 돌아간다. 자동 구매 팝업이 없다. 실제 판매 off 문구를 보여 준다.
4. 상세: 긴 중복 줄거리를 줄이고 상단 체험/재독 버튼을 배치했다. 아직 읽지 않은 장면에도 ‘체험이 끝났다’고 말하던 문구를 구매 조건으로 수정했다.

![서가의 무료 체험](screens/concept-review/11-pack-library.png)
![체험 종료와 돌아가기](screens/concept-review/12-pack-preview-end.png)

웹 미리보기의 합성 프로필 검증이며 Android 구매나 원어민 콘텐츠 검수를 의미하지 않는다. 유료 권한·두 결말·테마/표식 회수는 Flutter 상태/위젯 테스트로 검증했다. 별도의 Cloud/Billing-enabled 단위 테스트는 Play 경계만 가짜로 바꾸어 상품 조회 오류·재시도·중복 조회·조회 목록 외 SKU 제외를 확인했다. 실거래는 수행하지 않았다.

최종 v5 web 재시작에서도60XP와2/12 체험 기록이 유지됐다. 누적 현실 행동 조건과 다시 읽기 버튼을 확인했고412px 상세의 표지·줄거리·무료 체험·판매 상태를 다시 캡처했다.

![최신 팩 상세](screens/concept-review/13-pack-detail.png)
