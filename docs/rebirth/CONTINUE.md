...
4. Functions의 결제 검증/서버 권한/RTDN 환불/삭제 job을 실제 배포·테스트한다. 레거시 클라우드 계정도 요청 전 영속 guard와 재시작 복구 화면을 보강했다. 접수 후 정리 실패/응답 유실/저장 실패/4언어200% 글자 검사는15개 통과. 실제 서버 검증은 남아 있다. 원격 Firestore 인덱스를 보존하고 TTL만 안전하게 추가. Storage→Firestore 규칙 권한 및 실제 bucket 확인.
...
요청 전 UID별 guard를 저장하고 응답 유실 또는 기기 정리 실패 시 해당 프로필의 재로딩/저장을 차단한다. 서버가 수락한 요청은 로그아웃 실패 때문에 실패로 보고하지 않는다. 복구는 다른 계정/기기 전용 진행을 건드리지 않고 로컬 캐시/인증만 정리하며 서버 삭제를 다시 요청하지 않는다. 잘못된 응답은 거절로 단정하지 않고 보류한다. 계정 전환 후 이전 요청이 새 계정을 로그아웃하지 않도록 UID를 확인한다. 이 변경은 위 v4 바이너리 이후이며 새 릴리스 빌드 전까지 v4에 포함됐다고 표현하지 않는다.

## 콘셉트 이후 보강 · 조수 우체국 팩

12장면×4언어, 첫2장면 무조건 무료 체험, 누적 현실2~20개 조건, 앞선11선택의 다수결로2결말. image_gen 표지/투명 종이배 표식과 바다빛 테마를 구현했다. `TIDE_STORY_PACK.md` 참조. 현재 새 판매 조회 SKU는 story_tide_postoffice_01 하나이며 서버 allowlist와 일치한다. 실제 Play 상품은 미등록. 기본 결제/Cloud/광고 off를 유지한다.

권한 회수는 유료장면/테마/표식을 막지만 선택/XP는 보존한다. 백업은 묶음 테마 권한을 부여하지 않는다. 전체 Flutter330개와 새 팩 지급/환불 포함 Node22서버27개 통과. 별도 Cloud/Billing-enabled 상품 카탈로그 오류·재시도 검사1개도 통과(기본 전체 검사는 이1개만skip). 실제 CUA에서 체험 두 장면→이전 선택 반응→체험 종료→팩 상세 복귀를 확인했다. 무료 세 단편 선택도 보존된다. 현재 미리보기 합성 프로필에는 tide/door=open,tide/receipt=keep 기록이 추가됐다.

새 팩은 원고와 코드가 있는 출시 후보이며 독자/원어민 검수·가격 가치·실제 구매가 검증된 상품은 아니다. v4 바이너리에는 팩과 삭제 복구가 없다. 다음 최종 바이너리의 sourceCommit/해시를 새로 기록해야 한다. 추가 로컬 검토로 legacy cloud 쓰기 완료/계정 전환 문제와 게임 전체5구역 검증이 남아 있다.


## 콘셉트 이후 보강 · cloud 쓰기 완료와 계정 경계

각 저장 호출은 깊은 복사한 해당 시점 데이터가 실제 쓰기 경계를 통과할 때까지 기다린다. 이전 구현의 동시 저장 조기 반환과 예외 삼킴을 제거했다. 이미 로딩한 UID·프로필 세대·현재 인증이 일치해야 쓰며, 계정 전환/기기 모드 전환/삭제/해제 이후 대기 쓰기는 차단한다. 실패한 쓰기가 다음 쓰기를 막지 않고, 저장 실패 시 이야기 선택은 되돌리며 로그아웃은 중지한다. 홈 위젯 갱신 실패는 이미 성공한 클라우드 저장을 실패로 바꾸지 않는다.

신규7개 회귀 검사와 기존 검사를 합쳐 Flutter337개 통과, 기본 빌드에서만 결제 카탈로그 검사1개 skip(별도 Cloud/Billing 활성 검사에서 통과). 정적 분석 문제 없음. 실제 Firebase 연결은 미검증이며, 전체5구역 게임 밸런스/실물 QA도 계속 남아 있다. 이 소스를2.0.0+5로 빌드한다.
